...
const BirthRequests = () => {
  ...
  const fetchApplications = async () => {
    ...
    setSuccess('');
    try {
      const res = await apiRequest('/api/birth/all');
  ...
  const handleUpdateStatus = async (id) => {
    ...
    try {
      const res = await apiRequest(`/api/birth/${id}`, {
        method: 'PATCH',
        body: {
          status: statusAction,
  ...
  return (
    <div>
      ...
      {loading ? (
        <Loader message="Loading birth registrations..." />
      ) : applications.length === 0 ? (
        <div className="gov-card text-center" style={{ padding: '40px 20px' }}>
          <p style={{ fontSize: '15px', color: 'var(--light-text)' }}>No birth certificate applications found.</p>
        </div>
      ) : (
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '10px' }}>
                  <div>
                    ...
                    <h4 style={{ fontSize: '16px', fontWeight: 'bold', color: 'var(--secondary-blue)', textTransform: 'uppercase', margin: '3px 0' }}>
                      Child: {app.childName} ({app.gender})
                    </h4>
                    <span style={{ fontSize: '12px', color: 'var(--light-text)', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                      <Calendar size={12} />
                      Applicant: <strong>{citizen.name || 'Anonymous'}</strong> ({citizen.phone || 'No Phone'}) on {formatDate(app.createdAt)}
                    </span>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      {app.status === 'Approved' && (
                        <Link
                          to={`/citizen/certificate/birth/${app._id}`}
                ...
                {isExpanded && (
                  <div style={{ borderTop: '1px solid #e5e7eb', marginTop: '15px', paddingTop: '15px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '15px', backgroundColor: '#f9fafb', padding: '15px', borderRadius: '4px', border: '1px solid #e5e7eb' }}>
                      <div>
                        <span style={{ fontSize: '11px', color: 'var(--light-text)', fontWeight: 'bold' }}>DATE OF BIRTH</span>
                        <p style={{ fontWeight: 'bold', fontSize: '14px' }}>{formatDate(app.dateOfBirth)}</p>
                      </div>
                      <div>
                        <span style={{ fontSize: '11px', color: 'var(--light-text)', fontWeight: 'bold' }}>PLACE OF BIRTH</span>
                        <p style={{ fontWeight: 'bold', fontSize: '14px' }}>{app.placeOfBirth}</p>
                      </div>
                      <div>
                        <span style={{ fontSize: '11px', color: 'var(--light-text)', fontWeight: 'bold' }}>FATHER NAME</span>
                        <p style={{ fontWeight: 'bold', fontSize: '14px' }}>{app.fatherName}</p>
                      </div>
                      <div>
                        <span style={{ fontSize: '11px', color: 'var(--light-text)', fontWeight: 'bold' }}>MOTHER NAME</span>
                        <p style={{ fontWeight: 'bold', fontSize: '14px' }}>{app.motherName}</p>
                      </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '15px' }}>
                        ...
                        <div className="form-group">
                          ...
                          <input
                            ...
                            id="remarks"
                            className="form-control"
                            placeholder="Enter notes (e.g. Verified with hospital birth register entry #394)"
                            value={remarks}
